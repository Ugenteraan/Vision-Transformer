...
def main(gpu):
    ...
    TRAIN_DATALOADER = LoadDeeplakeDataset(token=cred.ACTIVELOOP_TOKEN, deeplake_ds_name="hub://activeloop/svhn-train", batch_size=cfg.BATCH_SIZE, shuffle=cfg.SHUFFLE, world_size=WORLD_SIZE, rank=RANK, num_workers=cfg.NUM_WORKERS, pin_memory=True)()
    TEST_DATALOADER = LoadDeeplakeDataset(token=cred.ACTIVELOOP_TOKEN, deeplake_ds_name="hub://activeloop/svhn-test", batch_size=cfg.BATCH_SIZE, shuffle=False, world_size=WORLD_SIZE, rank=RANK, num_workers=cfg.NUM_WORKERS, pin_memory=True)()
